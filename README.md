...
**📊 Метрика качества: Levenshtein mean — усредненное расстояние Левенштейна между истинными сообщениями и сообщениями, предсказанными моделью. Чем меньше расстояние, тем качественнее перевод.
...
Загрузка тестовых данных на Kaggle показала качество: **0.433** по усредненному расстоянию Левенштейна по тестовому датасету. Лучшая модель в лидерборде показала качество **0.25**.
![](Kagle.png)
## 📂 Структура проекта
```
morse_decoder/
├── 📂notebooks/  #Все неудачные попытки и 
│   ├── ...
...
Для улучшения качества модели были исследованы различные подходы к решению подобного рода задач (CTC, seq2seq, Transformers, ...), а также протестированы различные подходы к улучшению стандартных архитектур:
**- Улучшения CNN**
- [**SE blocks to CNN**](https://arxiv.org/pdf/2506.10683v1.pdf). Использования механизма внимания для CNN не дало прироста качества.

**- Улучшения RNN**
- Основным методом, улучшающим стандартные RNN-модели, является добавление модуля внимания (self-attention, multi-head attention). Применение attention-слоев не дало значительного прироста в качестве модели.
...
3. Дообучение/переобучение напрямую на сервере, с возможностью загрузки подготовленных данных на сервер по ссылке Google Drive.
